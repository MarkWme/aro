...
resource vmPublicIp 'Microsoft.Network/publicIPAddresses@2022-05-01' = {
  ...
  properties: {
    ...
    dnsSettings: {
      domainNameLabel: '${name}-dns-vm'
    }
  }
}

resource vmNic 'Microsoft.Network/networkInterfaces@2022-05-01' = {
  name: '${name}-dns-vm-nic'
  location: location
  properties: {
    ipConfigurations: [
      {
        name: '${name}-dns-vm-ipconfig'
        properties: {
